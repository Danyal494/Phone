...
import { BlenderSphere } from "./compressed";
...
const createTextTexture = (
  ...
  const someData =
    "am doloribus eos assumenda soluta iusto fugit perspiciatis quo, vitae vel sed";
  context.font = "normal 90px CustomFont";
  context.fillStyle = headingColor;
  ...
  context.font = "bold 80px CustomFont"; // Larger font size
  context.fillStyle = headingColor;
  context.fillText(buttonText, canvas.width / 2, buttonY + buttonHeight / 2);

  // Paragraph
  context.font = "80px CustomFont"; // Larger font size
  context.fillStyle = headingColor;
  context.textAlign = "center";
  ...
  useEffect(() => {
    if (controlsRef.current) {
      ...
      if (isLimitedZoom && !zoomLimitsSetRef.current) {
        ...
        console.log(
          `Min Distance: ${controlsRef.current.minDistance}, Max Distance: ${controlsRef.current.maxDistance}`
        );
        ...
        console.log(
          `Zoom limits reset: Min Distance: ${controlsRef.current.minDistance}, Max Distance: ${controlsRef.current.maxDistance}`
        );
      }
...
const Tasks = () => {
  const [heading, setHeading] = useState(
    "onsectetur adipisicing elit. Possimus aperiam hic sapiente, eos assumenda soluta iusto fugit perspiciatis quo, vitae vel sed. Harum eveniet sequi ut laudantium iure. Lorem  sapiente, quam doloribus eos assumenda soluta iusto fugit perspiciatis quo, vitae vel sed. Harum eveniet sequi ut laudantium iure."
  );
  ...
  const [rotateX, setRotateX] = useState(390.01);
  ...
  return (
    <div>
      {loading ? (
        ...
        <div>
          <Canvas
            camera={{ position: [0, 0, 0.1], fov: 10 }}
              ...
              <div>
                <div className="flex max-sm:left-0 max-sm:-top-[135px] max-sm:gap-0 max-sm: space-x-1 absolute bottom-48 left-[550px] transform -translate-x-1/2">
                  <div>
                    ...
                    >
                      <span
                        className="relative  flex items-center text-white font-bold
  ...
  max-sm:text-sm px-4 max-sm:px-1 py-2 max-sm:py-0"
                      >
                        ...
                        <Icon
                          className="mx-1"
                          icon="pepicons-print:color-picker"
                        />
                      </span>
                    </button>
                  ...
                  <select
                    value={envPreset}
                    onChange={handleEnvPresetChange}
                    className="inline-flex mt-2 p-2 w-28 hidden border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-slate-300"
                  >
  ...
  max-sm:text-sm px-4 max-sm:px-1 py-2 max-sm:py-0"
                    ...
                    <div className="absolute z-10 hidden group-hover:block  p-2 bg-white border border-gray-300 rounded shadow-lg">
                      <BlockPicker
                        className="custom-sketch-picker"
                        color={headingColor}
                        onChangeComplete={handleHeadingColorChange}
                      />
                    </div>
                  </div>
  ...
  max-sm:font-light max-sm:text-sm px-4 max-sm:px-1 py-2 max-sm:py-0"
                    >
                      {isLimitedZoom ? (
                        <Icon
                          className="h-[30px] max-sm:h-[10px]"
